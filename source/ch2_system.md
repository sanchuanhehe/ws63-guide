...
- **Light Sleep**：浅睡模式为可快速恢复业务收发的睡眠模式，关闭收发时钟以降低功耗。此时 CPU 配置为 WFI（Wait For Interrupt）模式，等待中断唤醒后恢复收发，Flash、IO 保持供电，系统内发生任意中断均可唤醒 CPU（如定时中断，外设通信中断，GPIO 中断等）；SoC 系统可选晶体或 PLL 时钟，时钟频率和外设的通信速率有关。
