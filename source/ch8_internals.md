...
**时钟树**：TCXO = 24 MHz（备选 40 MHz），TCXO tick 速率 24,000,000 /s；CPU 系统时钟 240 MHz（PLL，
boot ROM 设定）；PLL 为 FNPLL，配置寄存器 `FNPLL_CFG0..5` 位于 `0x4000_340C`～`0x4000_3420`；
时钟源选择 `CLK_SEL`（CLDO_CRG+0x34 = `0x4400_1134`，按外设 0=TCXO/1=PLL）。
...
表8-14 SFC 寄存器（flashboot 使用，绝对地址）
...
表8-16 闭源库目录（`ws63-RF/lib/`，共 14 个 `.a`，约 44 MB）
